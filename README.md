...
The commands rely on a file generated by the QDKP_V2 addon. To ensure the file remains up to date, you must manually back up the addon data, as shown in the image below.

![QDKP addon](/images/qdkpAddon.png)

The 'Backup' button instructs the game to write all data found in guild notes to the file. The 'Send Changes' button saves any modifications made to the guild notes. Therefore, you should first click 'Send Changes' to ensure all data is correctly updated in the guild notes. **Note: the data is only persisted in the files when the character logs out of the game**.
